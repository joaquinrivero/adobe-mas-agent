...
export const fetchConversations = async (user_id: string) => {
  console.log(`🔍 Attempting to fetch conversations for user: ${user_id}`);
  ...
  try {
    ...
    if (error) {
      // Enhanced error logging with clear markers
      ...
      console.log('STDOUT: Supabase Error Details:', JSON.stringify({
        error: error.message,
        code: error.code,
        hint: error.hint,
        details: error.details
    ...
    console.log(`✅ Successfully fetched ${data?.length || 0} conversations for user ${user_id}`);
    return data;
  } catch (error) {
...
export const fetchMessages = async (session_id: string, user_id: string) => {
  try {
    ...
    console.log(`Successfully fetched ${data?.length || 0} messages for session ${session_id}`);
