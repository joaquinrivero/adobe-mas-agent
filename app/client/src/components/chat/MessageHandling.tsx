...
export const useMessageHandling = ({
  ...
  const handleSendMessage = async (content: string, files?: FileAttachment[]) => {
    ...
    try {
      ...
      console.error('❌ Error in chat flow:', errorDetails);
  ...
  const loadMessages = useCallback(async (conversation: any) => {
    ...
    try {
      ...
      // Detailed error logging to stdout
      console.error('❌ Error loading messages:', errorDetails);
      console.log('STDOUT ERROR:', JSON.stringify(errorDetails, null, 2));

      // Extract user-friendly error message
      let errorMessage = 'Could not load conversation messages. Please try again later.';
