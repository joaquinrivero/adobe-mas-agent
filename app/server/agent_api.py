...
async def lifespan(app: FastAPI):
    ...
    # Startup: Initialize only essential clients
    # Mem0 will be initialized lazily on first use
    import sys
    sys.stdout.write("Initializing embedding client and Supabase...\n")
    sys.stdout.flush()
    ...
    # Don't initialize Mem0 during startup - it will be lazy loaded
    mem0_client = None

    sys.stdout.write("✓ Application startup complete (Mem0 will be lazy-loaded)\n")
    sys.stdout.flush()
...
async def pydantic_agent(request: AgentRequest, user: Dict[str, Any] = Depends(verify_token)):
    ...
    try:
        ...
        # Retrieve relevant memories with Mem0 (if available)
        memories_str = ""
        memory_task = None
        if mem0_client:
            try:
