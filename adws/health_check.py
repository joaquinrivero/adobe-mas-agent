...
3. Tests Claude Code CLI functionality (requires 'claude auth login')
4. Returns structured results

Note: Use 'claude auth login' for Claude authentication instead of ANTHROPIC_API_KEY
"""
...
def check_env_vars() -> CheckResult:
    ...
    # Note: ANTHROPIC_API_KEY is not required - use 'claude auth login' instead
...
def run_health_check() -> HealthCheckResult:
    ...
    # Check Claude Code (using 'claude auth login' for authentication)
...
def main():
    ...
    if not result.success:
        ...
        if any("Claude Code" in e for e in result.errors):
            print("   1. Authenticate with Claude: claude auth login")
        if any("GITHUB_PAT" in e for e in result.errors):
            print("   2. Set GITHUB_PAT in your .env file (optional)")
